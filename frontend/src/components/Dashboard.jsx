...
const Dashboard = () => {
  ...
  try {
    ...
    console.error('🚨 Error obteniendo AuthContext:', error);
  ...
  const loadDeliveries = useCallback(async () => {
    try {
      console.log('📂 Iniciando carga de deliveries...');
      setLoading(true);
      const result = await deliveryService.getActiveDeliveries();
      
      console.log('📋 Resultado de getActiveDeliveries:', result);
      
      if (result && result.success) {
        const deliveriesData = Array.isArray(result.data) ? result.data : [];
        console.log(`✅ ${deliveriesData.length} deliveries cargados`);
      ...
      console.error('💥 Error crítico cargando deliveries:', err);
      ...
      setLoading(false);
      console.log('🏁 Carga de deliveries finalizada');
    }
  }, []);
  ...
  useEffect(() => {
    if (socketConnected) {
      ...
      // Listener para actualizaciones múltiples de trips
      const handleTripsUpdate = (tripsData) => {
        console.log('🔄 Actualización de múltiples trips:', tripsData);
  ...
  const handleDeliveryAction = async (action, deliveryId) => {
    try {
      console.log(`🔄 Ejecutando acción '${action}' para delivery ${deliveryId}`);
      
      let result;
      
      if (action === 'start') {
        console.log('▶️ Iniciando viaje...');
        result = await deliveryService.startTrip(deliveryId);
      } else if (action === 'stop') {
        console.log('⏹️ Deteniendo viaje...');
      ...
      if (result?.success) {
        ...
        console.log('✅ Acción exitosa, recargando deliveries...');
        // Recargar deliveries
        await loadDeliveries();
      } else {
        console.error('❌ Error en el resultado:', result);
        toast.error(result?.error || 'Error en la acción');
      }
    } catch (error) {
      console.error('💥 Error crítico en acción de delivery:', error);
      console.error('Stack trace:', error.stack);
      
      // Mostrar error más específico
      const errorMessage = error.response?.data?.message || error.message || 'Error realizando la acción';
      ...
      try {
        ...
        console.error('Error recargando deliveries después del fallo:', reloadError);
      }
    }
  };
